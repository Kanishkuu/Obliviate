...
from apps.trainer.train import train
from apps.trainer.config import TrainConfig, LoRAConfig
from apps.settings import settings
from apps.api.progress import publish_progress

def rq_progress(job_id: str):
    def _cb(msg: Dict[str, Any]):
        publish_progress(job_id, msg)
    return _cb

def run_training_job(payload: Dict[str, Any]) -> Dict[str, Any]:
    run_id = payload.get("run_id") or str(uuid.uuid4())
    out_dir = os.path.join(settings.artifacts_dir, run_id)
    cfg = TrainConfig(
        run_id=run_id,
